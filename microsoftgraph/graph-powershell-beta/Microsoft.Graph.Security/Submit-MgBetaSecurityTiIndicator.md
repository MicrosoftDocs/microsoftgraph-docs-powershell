...
Read-only.
    ...
    \[AdditionalInformation \<String\>\]: A catchall area into which extra data from the indicator not covered by the other tiIndicator properties may be placed.
...
Required.
    \[ExternalId \<String\>\]: An identification number that ties the indicator back to the indicator provider's system (e.g.
a foreign key).
