...
$EmsSku = Get-MgBetaSubscribedSku -All ### Example 1: Assign a license to a user### Example 2: Assign more than one licenses to a user

```powershell
Connect-Graph -Scopes User.ReadWrite.All, Organization.Read.All

$EmsSku = Get-MgBetaSubscribedSku -All  Where SkuPartNumber -eq 'FLOW_FREE'
$addLicenses = @(
  @{SkuId = $EmsSku.SkuId},
  @{SkuId = $FlowSku.SkuId}
  )
...
$EmsSku = Get-MgBetaSubscribedSku -All  where ServicePlanName -in ("MFA_PREMIUM", "INTUNE_A") ### Example 4: Update a license assigned to a user to add more disabled plans leaving the user's existing disabled plans in their current state

```powershell
Connect-Graph -Scopes User.ReadWrite.All, Organization.Read.All

$EmsSku = Get-MgBetaSubscribedSku -All 
...
$EmsSku = Get-MgBetaSubscribedSku -All ### Example 6: Remove a license assigned to a user
## PARAMETERS
