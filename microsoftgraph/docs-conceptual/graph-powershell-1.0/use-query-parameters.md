...
|----|----|-----|
|[$count](#count-parameter)  |Retrieves the total count of matching resources|`Get-MgUser -ConsistencyLevel eventual -Count count`,`$count`|
| [$expand](#expand-parameter)       | Retrieves related resources.||
| [$filter](#filter-parameter)       | Filters results (rows).||
| [$format](#format-parameter)       | Returns the results in the specified media format.|
| [$orderby](#orderby-parameter)     | Orders results.|
| [$search](#search-parameter)       | Returns results based on search criteria. |
| [$select](#select-parameter)       | Filters properties (columns).|
...
| [$top](#top-parameter)             | Sets the page size of results. |
...
For example, the following command returns all the users and store their count in the variable `$count`. Querying the variable should return the count of all the users.
