...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`tetermstoresettermch`i`ldrelat`i`onfromterm
schema: 2.0.0
---

# Get-MgS`i`teTermStoreSetTermCh`i`ldRelat`i`onFromTerm
...
### Get (Default)
```
Get-MgS`i`teTermStoreSetTermCh`i`ldRelat`i`onFromTerm -Relat`i`on`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng>
...
```
Get-MgS`i`teTermStoreSetTermCh`i`ldRelat`i`onFromTerm -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
