...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mggroups`i`tetermstoresettermch`i`ldrelat`i`ontoterm
schema: 2.0.0
---

# Get-MgGroupS`i`teTermStoreSetTermCh`i`ldRelat`i`onToTerm
...
### Get (Default)
```
Get-MgGroupS`i`teTermStoreSetTermCh`i`ldRelat`i`onToTerm -Group`i`d <Str`i`ng> -Relat`i`on`i`d <Str`i`ng> -Set`i`d <Str`i`ng>
...
```
Get-MgGroupS`i`teTermStoreSetTermCh`i`ldRelat`i`onToTerm -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
 [-Property <Str`i`ng[]>] [<CommonParameters>]
```
