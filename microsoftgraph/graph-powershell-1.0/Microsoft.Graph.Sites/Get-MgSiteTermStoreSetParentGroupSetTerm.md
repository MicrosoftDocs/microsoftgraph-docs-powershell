...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`tetermstoresetparentgroupsetterm
schema: 2.0.0
---

# Get-MgS`i`teTermStoreSetParentGroupSetTerm
...
### L`i`st (Default)
```
Get-MgS`i`teTermStoreSetParentGroupSetTerm -Set`i`d <Str`i`ng> -Set`i`d1 <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng>
...
```
Get-MgS`i`teTermStoreSetParentGroupSetTerm -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
