...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`tetermstoregroupsettermch`i`ld
schema: 2.0.0
---

# Get-MgS`i`teTermStoreGroupSetTermCh`i`ld
...
### L`i`st2 (Default)
```
Get-MgS`i`teTermStoreGroupSetTermCh`i`ld -Group`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Term`i`d <Str`i`ng>
...
### L`i`st
```
Get-MgS`i`teTermStoreGroupSetTermCh`i`ld -Group`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng>
...
```
Get-MgS`i`teTermStoreGroupSetTermCh`i`ld -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
