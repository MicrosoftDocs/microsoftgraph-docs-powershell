...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mggroups`i`tel`i`st`i`temvers`i`onf`i`eld
schema: 2.0.0
---

# Get-MgGroupS`i`teL`i`st`i`temVers`i`onF`i`eld
...
### Get (Default)
```
Get-MgGroupS`i`teL`i`st`i`temVers`i`onF`i`eld -Group`i`d <Str`i`ng> -L`i`st`i`d <Str`i`ng> -L`i`st`i`tem`i`d <Str`i`ng>
...
```
Get-MgGroupS`i`teL`i`st`i`temVers`i`onF`i`eld -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
