...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mggroups`i`tetermstoresetparentgroupsettermset
schema: 2.0.0
---

# Get-MgGroupS`i`teTermStoreSetParentGroupSetTermSet
...
### Get (Default)
```
Get-MgGroupS`i`teTermStoreSetParentGroupSetTermSet -Group`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -Set`i`d1 <Str`i`ng>
...
```
Get-MgGroupS`i`teTermStoreSetParentGroupSetTermSet -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
