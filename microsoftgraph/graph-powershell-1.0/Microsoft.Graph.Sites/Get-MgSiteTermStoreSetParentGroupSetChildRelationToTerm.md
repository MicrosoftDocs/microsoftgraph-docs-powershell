...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`tetermstoresetparentgroupsetch`i`ldrelat`i`ontoterm
schema: 2.0.0
---

# Get-MgS`i`teTermStoreSetParentGroupSetCh`i`ldRelat`i`onToTerm
...
### Get1 (Default)
```
Get-MgS`i`teTermStoreSetParentGroupSetCh`i`ldRelat`i`onToTerm -Relat`i`on`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -Set`i`d1 <Str`i`ng>
...
```
Get-MgS`i`teTermStoreSetParentGroupSetCh`i`ldRelat`i`onToTerm -`i`nputObject <`i`S`i`tes`i`dent`i`ty>
