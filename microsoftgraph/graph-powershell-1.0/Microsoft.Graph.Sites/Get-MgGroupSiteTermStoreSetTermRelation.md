...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mggroups`i`tetermstoresettermrelat`i`on
schema: 2.0.0
---

# Get-MgGroupS`i`teTermStoreSetTermRelat`i`on
...
### L`i`st1 (Default)
```
Get-MgGroupS`i`teTermStoreSetTermRelat`i`on -Group`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Term`i`d <Str`i`ng>
...
### Get2
```
Get-MgGroupS`i`teTermStoreSetTermRelat`i`on -Group`i`d <Str`i`ng> -Relat`i`on`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng>
...
```
Get-MgGroupS`i`teTermStoreSetTermRelat`i`on -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
