...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`tetermstoresetch`i`ldrelat`i`on
schema: 2.0.0
---

# Get-MgS`i`teTermStoreSetCh`i`ldRelat`i`on
...
### L`i`st1 (Default)
```
Get-MgS`i`teTermStoreSetCh`i`ldRelat`i`on -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng> -Term`i`d <Str`i`ng>
...
### Get1
```
Get-MgS`i`teTermStoreSetCh`i`ldRelat`i`on -Relat`i`on`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng>
...
```
Get-MgS`i`teTermStoreSetCh`i`ldRelat`i`on -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
