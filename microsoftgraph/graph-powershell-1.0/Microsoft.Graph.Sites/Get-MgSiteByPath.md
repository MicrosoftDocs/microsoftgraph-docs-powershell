...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`tebypath
schema: 2.0.0
---
...
## SYNOPS`i`S
`i`nvoke funct`i`on getByPath

## SYNTAX

### Get1 (Default)
```
Get-MgS`i`teByPath -Path <Str`i`ng> -S`i`te`i`d <Str`i`ng> [<CommonParameters>]
```

### GetV`i`a`i`dent`i`ty1
```
Get-MgS`i`teByPath -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [<CommonParameters>]
```
