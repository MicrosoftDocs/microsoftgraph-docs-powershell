...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mggroups`i`tetermstoregroupset
schema: 2.0.0
---

# Remove-MgGroupS`i`teTermStoreGroupSet

## SYNOPS`i`S
Delete nav`i`gat`i`on property sets for groups

## SYNTAX

### Delete1 (Default)
```
Remove-MgGroupS`i`teTermStoreGroupSet -Group`i`d <Str`i`ng> -Group`i`d1 <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng>
...
```
Remove-MgGroupS`i`teTermStoreGroupSet -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f]
