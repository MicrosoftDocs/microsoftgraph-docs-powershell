...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mggroups`i`tel`i`st
schema: 2.0.0
---

# Remove-MgGroupS`i`teL`i`st

## SYNOPS`i`S
Delete nav`i`gat`i`on property l`i`sts for groups

## SYNTAX

### Delete1 (Default)
```
Remove-MgGroupS`i`teL`i`st -Group`i`d <Str`i`ng> -L`i`st`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> [-`i`fMatch <Str`i`ng>] [-PassThru]
 [-What`i`f] [-Conf`i`rm] [<CommonParameters>]
```

### DeleteV`i`a`i`dent`i`ty1
```
Remove-MgGroupS`i`teL`i`st -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f] [-Conf`i`rm]
 [<CommonParameters>]
```
