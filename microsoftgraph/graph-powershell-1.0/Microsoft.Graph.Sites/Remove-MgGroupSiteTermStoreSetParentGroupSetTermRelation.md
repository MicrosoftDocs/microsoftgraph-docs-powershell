...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mggroups`i`tetermstoresetparentgroupsettermrelat`i`on
schema: 2.0.0
---

# Remove-MgGroupS`i`teTermStoreSetParentGroupSetTermRelat`i`on
...
### Delete (Default)
```
Remove-MgGroupS`i`teTermStoreSetParentGroupSetTermRelat`i`on -Group`i`d <Str`i`ng> -Relat`i`on`i`d <Str`i`ng> -Set`i`d <Str`i`ng>
 -Set`i`d1 <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng> -Term`i`d <Str`i`ng> [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f]
 [-Conf`i`rm] [<CommonParameters>]
```

### DeleteV`i`a`i`dent`i`ty
```
Remove-MgGroupS`i`teTermStoreSetParentGroupSetTermRelat`i`on -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>]
