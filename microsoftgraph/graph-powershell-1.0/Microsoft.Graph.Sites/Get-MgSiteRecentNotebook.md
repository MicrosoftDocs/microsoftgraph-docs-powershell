...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`terecentnotebook
schema: 2.0.0
---

# Get-MgS`i`teRecentNotebook

## SYNOPS`i`S
`i`nvoke funct`i`on getRecentNotebooks

## SYNTAX

### Get1 (Default)
```
Get-MgS`i`teRecentNotebook [-`i`ncludePersonalNotebooks] -S`i`te`i`d <Str`i`ng> [<CommonParameters>]
```

### GetV`i`a`i`dent`i`ty1
```
Get-MgS`i`teRecentNotebook -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [<CommonParameters>]
```
...
### -`i`ncludePersonalNotebooks
Usage: `i`ncludePersonalNotebooks={`i`ncludePersonalNotebooks}
...
### M`i`crosoft.Graph.PowerShell.Models.`i`M`i`crosoftGraphRecentNotebook
## NOTES
