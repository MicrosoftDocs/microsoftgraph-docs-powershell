...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mggroups`i`tel`i`st`i`temvers`i`on
schema: 2.0.0
---

# Remove-MgGroupS`i`teL`i`st`i`temVers`i`on
...
### Delete (Default)
```
Remove-MgGroupS`i`teL`i`st`i`temVers`i`on -Group`i`d <Str`i`ng> -L`i`st`i`d <Str`i`ng> -L`i`st`i`tem`i`d <Str`i`ng>
...
```
Remove-MgGroupS`i`teL`i`st`i`temVers`i`on -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f]
