...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/unpubl`i`sh-mgs`i`tecontenttype
schema: 2.0.0
---

# Unpubl`i`sh-MgS`i`teContentType
...
```
Unpubl`i`sh-MgS`i`teContentType -ContentType`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> [-PassThru] [-What`i`f] [-Conf`i`rm]
 [<CommonParameters>]
```

### Unpubl`i`shV`i`a`i`dent`i`ty1
```
Unpubl`i`sh-MgS`i`teContentType -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-PassThru] [-What`i`f] [-Conf`i`rm] [<CommonParameters>]
```
