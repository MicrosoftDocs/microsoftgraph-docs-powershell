...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`tetermstoregroup
schema: 2.0.0
---

# Get-MgS`i`teTermStoreGroup

## SYNOPS`i`S
Get groups from s`i`tes

## SYNTAX

### L`i`st1 (Default)
```
Get-MgS`i`teTermStoreGroup -S`i`te`i`d <Str`i`ng> [-ExpandProperty <Str`i`ng[]>] [-Property <Str`i`ng[]>]
...
### Get2
```
Get-MgS`i`teTermStoreGroup -Group`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng> [-ExpandProperty <Str`i`ng[]>]
 [-Property <Str`i`ng[]>] [<CommonParameters>]
```

### Get1
```
Get-MgS`i`teTermStoreGroup -Group`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> [-ExpandProperty <Str`i`ng[]>] [-Property <Str`i`ng[]>]
 [<CommonParameters>]
```

### L`i`st2
```
Get-MgS`i`teTermStoreGroup -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng> [-ExpandProperty <Str`i`ng[]>] [-Property <Str`i`ng[]>]
 [-F`i`lter <Str`i`ng>] [-Search <Str`i`ng>] [-Sk`i`p <`i`nt32>] [-Sort <Str`i`ng[]>] [-Top <`i`nt32>] [-PageS`i`ze <`i`nt32>]
 [-All] [-CountVar`i`able <Str`i`ng>] [<CommonParameters>]
```
...
```
Get-MgS`i`teTermStoreGroup -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>] [-Property <Str`i`ng[]>]
 [<CommonParameters>]
```
...
```yaml
Type: Str`i`ng
Parameter Sets: L`i`st1, Get2, Get1, L`i`st2
Al`i`ases:
