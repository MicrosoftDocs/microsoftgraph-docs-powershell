...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mggroups`i`tel`i`st`i`temdr`i`ve`i`temcontent
schema: 2.0.0
---

# Get-MgGroupS`i`teL`i`st`i`temDr`i`ve`i`temContent
...
### Get (Default)
```
Get-MgGroupS`i`teL`i`st`i`temDr`i`ve`i`temContent -Group`i`d <Str`i`ng> -L`i`st`i`d <Str`i`ng> -L`i`st`i`tem`i`d <Str`i`ng>
 -S`i`te`i`d <Str`i`ng> -OutF`i`le <Str`i`ng> [-PassThru] [<CommonParameters>]
```

### GetV`i`a`i`dent`i`ty
```
Get-MgGroupS`i`teL`i`st`i`temDr`i`ve`i`temContent -`i`nputObject <`i`S`i`tes`i`dent`i`ty> -OutF`i`le <Str`i`ng> [-PassThru]
 [<CommonParameters>]
```
