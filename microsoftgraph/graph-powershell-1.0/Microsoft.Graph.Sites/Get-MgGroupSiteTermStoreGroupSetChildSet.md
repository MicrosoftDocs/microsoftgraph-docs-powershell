...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mggroups`i`tetermstoregroupsetch`i`ldset
schema: 2.0.0
---

# Get-MgGroupS`i`teTermStoreGroupSetCh`i`ldSet
...
### Get1 (Default)
```
Get-MgGroupS`i`teTermStoreGroupSetCh`i`ldSet -Group`i`d <Str`i`ng> -Group`i`d1 <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng>
...
```
Get-MgGroupS`i`teTermStoreGroupSetCh`i`ldSet -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
