...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mggroups`i`tel`i`stcontenttypecolumnl`i`nk
schema: 2.0.0
---

# Remove-MgGroupS`i`teL`i`stContentTypeColumnL`i`nk
...
### Delete (Default)
```
Remove-MgGroupS`i`teL`i`stContentTypeColumnL`i`nk -ColumnL`i`nk`i`d <Str`i`ng> -ContentType`i`d <Str`i`ng> -Group`i`d <Str`i`ng>
...
```
Remove-MgGroupS`i`teL`i`stContentTypeColumnL`i`nk -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>] [-PassThru]
