...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`teappl`i`cablecontenttypeforl`i`st
schema: 2.0.0
---

# Get-MgS`i`teAppl`i`cableContentTypeForL`i`st

## SYNOPS`i`S
`i`nvoke funct`i`on getAppl`i`cableContentTypesForL`i`st

## SYNTAX

### Get1 (Default)
```
Get-MgS`i`teAppl`i`cableContentTypeForL`i`st -L`i`st`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> [<CommonParameters>]
```

### GetV`i`a`i`dent`i`ty1
```
Get-MgS`i`teAppl`i`cableContentTypeForL`i`st -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [<CommonParameters>]
```
