...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mgs`i`tetermstoresetparentgroupsetch`i`ldrelat`i`on
schema: 2.0.0
---

# Remove-MgS`i`teTermStoreSetParentGroupSetCh`i`ldRelat`i`on
...
### Delete1 (Default)
```
Remove-MgS`i`teTermStoreSetParentGroupSetCh`i`ldRelat`i`on -Relat`i`on`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -Set`i`d1 <Str`i`ng>
...
```
Remove-MgS`i`teTermStoreSetParentGroupSetCh`i`ldRelat`i`on -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>]
