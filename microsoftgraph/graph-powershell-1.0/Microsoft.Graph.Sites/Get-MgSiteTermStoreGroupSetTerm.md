...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`tetermstoregroupsetterm
schema: 2.0.0
---

# Get-MgS`i`teTermStoreGroupSetTerm

## SYNOPS`i`S
Get terms from s`i`tes

## SYNTAX

### L`i`st2 (Default)
```
Get-MgS`i`teTermStoreGroupSetTerm -Group`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> [-ExpandProperty <Str`i`ng[]>]
...
### L`i`st
```
Get-MgS`i`teTermStoreGroupSetTerm -Group`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng>
...
### Get2
```
Get-MgS`i`teTermStoreGroupSetTerm -Group`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Term`i`d <Str`i`ng>
...
```
Get-MgS`i`teTermStoreGroupSetTerm -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
...
```yaml
Type: Str`i`ng
Parameter Sets: L`i`st2, L`i`st, Get2, Get
Al`i`ases:
...
Parameter Sets: GetV`i`a`i`dent`i`ty2, GetV`i`a`i`dent`i`ty
Al`i`ases:
