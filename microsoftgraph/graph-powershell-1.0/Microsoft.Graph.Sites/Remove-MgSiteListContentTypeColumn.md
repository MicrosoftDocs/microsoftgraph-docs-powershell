...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mgs`i`tel`i`stcontenttypecolumn
schema: 2.0.0
---

# Remove-MgS`i`teL`i`stContentTypeColumn
...
### Delete1 (Default)
```
Remove-MgS`i`teL`i`stContentTypeColumn -ColumnDef`i`n`i`t`i`on`i`d <Str`i`ng> -ContentType`i`d <Str`i`ng> -L`i`st`i`d <Str`i`ng>
 -S`i`te`i`d <Str`i`ng> [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f] [-Conf`i`rm] [<CommonParameters>]
```

### DeleteV`i`a`i`dent`i`ty1
```
Remove-MgS`i`teL`i`stContentTypeColumn -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f]
