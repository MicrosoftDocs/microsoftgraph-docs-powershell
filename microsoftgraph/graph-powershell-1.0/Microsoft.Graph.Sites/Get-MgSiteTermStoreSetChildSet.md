...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`tetermstoresetch`i`ldset
schema: 2.0.0
---

# Get-MgS`i`teTermStoreSetCh`i`ldSet
...
### Get1 (Default)
```
Get-MgS`i`teTermStoreSetCh`i`ldSet -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng> -Term`i`d <Str`i`ng>
...
```
Get-MgS`i`teTermStoreSetCh`i`ldSet -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
