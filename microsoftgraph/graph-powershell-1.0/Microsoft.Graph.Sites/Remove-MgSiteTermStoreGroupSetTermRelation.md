...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mgs`i`tetermstoregroupsettermrelat`i`on
schema: 2.0.0
---

# Remove-MgS`i`teTermStoreGroupSetTermRelat`i`on
...
### Delete2 (Default)
```
Remove-MgS`i`teTermStoreGroupSetTermRelat`i`on -Group`i`d <Str`i`ng> -Relat`i`on`i`d <Str`i`ng> -Set`i`d <Str`i`ng>
...
```
Remove-MgS`i`teTermStoreGroupSetTermRelat`i`on -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>] [-PassThru]
