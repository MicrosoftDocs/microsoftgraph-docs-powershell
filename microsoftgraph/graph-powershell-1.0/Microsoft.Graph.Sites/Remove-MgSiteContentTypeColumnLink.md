...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mgs`i`tecontenttypecolumnl`i`nk
schema: 2.0.0
---

# Remove-MgS`i`teContentTypeColumnL`i`nk
...
### Delete1 (Default)
```
Remove-MgS`i`teContentTypeColumnL`i`nk -ColumnL`i`nk`i`d <Str`i`ng> -ContentType`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng>
 [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f] [-Conf`i`rm] [<CommonParameters>]
```

### DeleteV`i`a`i`dent`i`ty1
```
Remove-MgS`i`teContentTypeColumnL`i`nk -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f]
