...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`tetermstoresetparentgroupsettermrelat`i`onset
schema: 2.0.0
---

# Get-MgS`i`teTermStoreSetParentGroupSetTermRelat`i`onSet
...
### Get (Default)
```
Get-MgS`i`teTermStoreSetParentGroupSetTermRelat`i`onSet -Relat`i`on`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -Set`i`d1 <Str`i`ng>
...
```
Get-MgS`i`teTermStoreSetParentGroupSetTermRelat`i`onSet -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
