...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mgs`i`tetermstoregroupsetrelat`i`on
schema: 2.0.0
---

# Remove-MgS`i`teTermStoreGroupSetRelat`i`on

## SYNOPS`i`S
Delete nav`i`gat`i`on property relat`i`ons for s`i`tes

## SYNTAX

### Delete2 (Default)
```
Remove-MgS`i`teTermStoreGroupSetRelat`i`on -Group`i`d <Str`i`ng> -Relat`i`on`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng>
...
```
Remove-MgS`i`teTermStoreGroupSetRelat`i`on -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f]
