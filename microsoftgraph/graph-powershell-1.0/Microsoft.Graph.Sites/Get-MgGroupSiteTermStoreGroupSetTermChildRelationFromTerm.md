...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mggroups`i`tetermstoregroupsettermch`i`ldrelat`i`onfromterm
schema: 2.0.0
---

# Get-MgGroupS`i`teTermStoreGroupSetTermCh`i`ldRelat`i`onFromTerm
...
### Get (Default)
```
Get-MgGroupS`i`teTermStoreGroupSetTermCh`i`ldRelat`i`onFromTerm -Group`i`d <Str`i`ng> -Group`i`d1 <Str`i`ng>
...
```
Get-MgGroupS`i`teTermStoreGroupSetTermCh`i`ldRelat`i`onFromTerm -`i`nputObject <`i`S`i`tes`i`dent`i`ty>
 [-ExpandProperty <Str`i`ng[]>] [-Property <Str`i`ng[]>] [<CommonParameters>]
```
