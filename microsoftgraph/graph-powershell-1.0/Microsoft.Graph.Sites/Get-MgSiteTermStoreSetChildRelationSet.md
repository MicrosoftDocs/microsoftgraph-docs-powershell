...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mgs`i`tetermstoresetch`i`ldrelat`i`onset
schema: 2.0.0
---

# Get-MgS`i`teTermStoreSetCh`i`ldRelat`i`onSet
...
### Get1 (Default)
```
Get-MgS`i`teTermStoreSetCh`i`ldRelat`i`onSet -Relat`i`on`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng>
...
```
Get-MgS`i`teTermStoreSetCh`i`ldRelat`i`onSet -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
