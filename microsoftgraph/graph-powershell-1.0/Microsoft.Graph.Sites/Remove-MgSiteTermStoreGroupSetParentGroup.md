...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mgs`i`tetermstoregroupsetparentgroup
schema: 2.0.0
---

# Remove-MgS`i`teTermStoreGroupSetParentGroup

## SYNOPS`i`S
Delete nav`i`gat`i`on property parentGroup for s`i`tes

## SYNTAX

### Delete (Default)
```
Remove-MgS`i`teTermStoreGroupSetParentGroup -Group`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng>
 [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f] [-Conf`i`rm] [<CommonParameters>]
```

### DeleteV`i`a`i`dent`i`ty
```
Remove-MgS`i`teTermStoreGroupSetParentGroup -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>] [-PassThru]
