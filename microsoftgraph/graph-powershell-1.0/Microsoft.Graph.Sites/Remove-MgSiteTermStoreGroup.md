...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/remove-mgs`i`tetermstoregroup
schema: 2.0.0
---

# Remove-MgS`i`teTermStoreGroup

## SYNOPS`i`S
Delete nav`i`gat`i`on property groups for s`i`tes

## SYNTAX

### Delete2 (Default)
```
Remove-MgS`i`teTermStoreGroup -Group`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f]
 [-Conf`i`rm] [<CommonParameters>]
```

### Delete
```
Remove-MgS`i`teTermStoreGroup -Group`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng> [-`i`fMatch <Str`i`ng>]
 [-PassThru] [-What`i`f] [-Conf`i`rm] [<CommonParameters>]
```

### DeleteV`i`a`i`dent`i`ty2
```
Remove-MgS`i`teTermStoreGroup -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-`i`fMatch <Str`i`ng>] [-PassThru] [-What`i`f] [-Conf`i`rm]
 [<CommonParameters>]
```
...
Parameter Sets: DeleteV`i`a`i`dent`i`ty2, DeleteV`i`a`i`dent`i`ty
Al`i`ases:
