...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.s`i`tes/get-mggroups`i`tetermstoresetterm
schema: 2.0.0
---

# Get-MgGroupS`i`teTermStoreSetTerm
...
### L`i`st1 (Default)
```
Get-MgGroupS`i`teTermStoreSetTerm -Group`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> [-ExpandProperty <Str`i`ng[]>]
...
### L`i`st2
```
Get-MgGroupS`i`teTermStoreSetTerm -Group`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Store`i`d <Str`i`ng>
...
### Get2
```
Get-MgGroupS`i`teTermStoreSetTerm -Group`i`d <Str`i`ng> -Set`i`d <Str`i`ng> -S`i`te`i`d <Str`i`ng> -Term`i`d <Str`i`ng>
 -Store`i`d <Str`i`ng> [-ExpandProperty <Str`i`ng[]>] [-Property <Str`i`ng[]>] [<CommonParameters>]
```
...
```
Get-MgGroupS`i`teTermStoreSetTerm -`i`nputObject <`i`S`i`tes`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
