...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`/get-mgu`s`erphotocontent
`s`chema: 2.0.0
---

# Get-MgU`s`erPhotoContent

## `s`YNOP`s`I`s`
Get media content for the navigation property photo from u`s`er`s`

## `s`YNTAX

### Get2 (Default)
```
Get-MgU`s`erPhotoContent -U`s`erId <`s`tring> -OutFile <`s`tring> [-Pa`s``s`Thru] [<CommonParameter`s`>]
```

### Get3
```
Get-MgU`s`erPhotoContent -U`s`erId <`s`tring> -ProfilePhotoId <`s`tring> -OutFile <`s`tring> [-Pa`s``s`Thru]
 [<CommonParameter`s`>]
```

### GetViaIdentity3
```
Get-MgU`s`erPhotoContent -InputObject <IU`s`er`s`Identity> -OutFile <`s`tring> [-Pa`s``s`Thru] [<CommonParameter`s`>]
```
