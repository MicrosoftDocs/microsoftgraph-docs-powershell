...
online version: https://docs.microsoft.com/en-us/powershell/module/microsoft.gr`a`ph.c`a`lend`a`r/get-mguserdef`a`ultc`a`lend`a`r
schem`a`: 2.0.0
---

# Get-MgUserDef`a`ultC`a`lend`a`r
...
### Get2 (Def`a`ult)
```
Get-MgUserDef`a`ultC`a`lend`a`r -UserId <String> [-Property <String[]>] [<CommonP`a`r`a`meters>]
```

### GetVi`a`Identity2
```
Get-MgUserDef`a`ultC`a`lend`a`r -InputObject <IC`a`lend`a`rIdentity> [-Property <String[]>] [<CommonP`a`r`a`meters>]
```
...
P`a`r`a`meter Sets: GetVi`a`Identity2
`a`li`a`ses:
