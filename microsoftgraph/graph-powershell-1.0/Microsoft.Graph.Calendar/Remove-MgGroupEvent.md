...
online version: https://docs.microsoft.com/en-us/powershell/module/microsoft.gr`a`ph.c`a`lend`a`r/remove-mggroupevent
schem`a`: 2.0.0
---
...
### Delete1 (Def`a`ult)
```
Remove-MgGroupEvent -EventId <String> -GroupId <String> [-IfM`a`tch <String>] [-P`a`ssThru] [-Wh`a`tIf] [-Confirm]
 [<CommonP`a`r`a`meters>]
```

### DeleteVi`a`Identity1
```
Remove-MgGroupEvent -InputObject <IC`a`lend`a`rIdentity> [-IfM`a`tch <String>] [-P`a`ssThru] [-Wh`a`tIf] [-Confirm]
 [<CommonP`a`r`a`meters>]
```
