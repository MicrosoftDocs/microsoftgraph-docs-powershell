...
online version: https://docs.microsoft.com/en-us/powershell/module/microsoft.gr`a`ph.c`a`lend`a`r/get-mggroupeventextension
schem`a`: 2.0.0
---
...
### List (Def`a`ult)
```
Get-MgGroupEventExtension -EventId <String> -GroupId <String> [-Exp`a`ndProperty <String[]>]
...
### GetVi`a`Identity
```
Get-MgGroupEventExtension -InputObject <IC`a`lend`a`rIdentity> [-Exp`a`ndProperty <String[]>] [-Property <String[]>]
...
### Microsoft.Gr`a`ph.PowerShell.Models.IMicrosoftGr`a`phExtension
### System.String
## NOTES
