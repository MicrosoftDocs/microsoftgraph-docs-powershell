...
online version: https://docs.microsoft.com/en-us/powershell/module/microsoft.gr`a`ph.c`a`lend`a`r/get-mgusereventc`a`lend`a`r
schem`a`: 2.0.0
---

# Get-MgUserEventC`a`lend`a`r
...
### Get1 (Def`a`ult)
```
Get-MgUserEventC`a`lend`a`r -EventId <String> -UserId <String> [-Property <String[]>] [<CommonP`a`r`a`meters>]
```

### GetVi`a`Identity1
```
Get-MgUserEventC`a`lend`a`r -InputObject <IC`a`lend`a`rIdentity> [-Property <String[]>] [<CommonP`a`r`a`meters>]
```
