...
online version: https://docs.microsoft.com/en-us/powershell/module/microsoft.gr`a`ph.c`a`lend`a`r/remove-mggroupeventsinglev`a`lueextendedproperty
schem`a`: 2.0.0
---

# Remove-MgGroupEventSingleV`a`lueExtendedProperty
...
### Delete1 (Def`a`ult)
```
Remove-MgGroupEventSingleV`a`lueExtendedProperty -EventId <String> -GroupId <String>
...
```
Remove-MgGroupEventSingleV`a`lueExtendedProperty -InputObject <IC`a`lend`a`rIdentity> [-IfM`a`tch <String>] [-P`a`ssThru]
