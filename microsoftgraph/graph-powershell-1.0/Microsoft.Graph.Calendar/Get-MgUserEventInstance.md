...
online version: https://docs.microsoft.com/en-us/powershell/module/microsoft.gr`a`ph.c`a`lend`a`r/get-mgusereventinst`a`nce
schem`a`: 2.0.0
---

# Get-MgUserEventInst`a`nce
...
### List1 (Def`a`ult)
```
Get-MgUserEventInst`a`nce -EventId <String> -UserId <String> [-Property <String[]>] [-Filter <String>]
...
### Get1
```
Get-MgUserEventInst`a`nce -EventId <String> -EventId1 <String> -UserId <String> [-Property <String[]>]
 [<CommonP`a`r`a`meters>]
```

### GetVi`a`Identity1
```
Get-MgUserEventInst`a`nce -InputObject <IC`a`lend`a`rIdentity> [-Property <String[]>] [<CommonP`a`r`a`meters>]
```
