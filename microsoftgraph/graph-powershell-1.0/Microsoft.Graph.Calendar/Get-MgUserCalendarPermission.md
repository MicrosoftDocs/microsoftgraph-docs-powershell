...
online version: https://docs.microsoft.com/en-us/powershell/module/microsoft.gr`a`ph.c`a`lend`a`r/get-mguserc`a`lend`a`rpermission
schem`a`: 2.0.0
---

# Get-MgUserC`a`lend`a`rPermission
...
### List2 (Def`a`ult)
```
Get-MgUserC`a`lend`a`rPermission -UserId <String> [-Property <String[]>] [-Filter <String>] [-Skip <Int32>]
 [-Sort <String[]>] [-Top <Int32>] [-P`a`geSize <Int32>] [-`a`ll] [-CountV`a`ri`a`ble <String>] [<CommonP`a`r`a`meters>]
```

### Get3
```
Get-MgUserC`a`lend`a`rPermission -C`a`lend`a`rPermissionId <String> -UserId <String> -C`a`lend`a`rId <String>
 [-Property <String[]>] [<CommonP`a`r`a`meters>]
```

### Get2
```
Get-MgUserC`a`lend`a`rPermission -C`a`lend`a`rPermissionId <String> -UserId <String> [-Property <String[]>]
 [<CommonP`a`r`a`meters>]
```

### List3
```
Get-MgUserC`a`lend`a`rPermission -UserId <String> -C`a`lend`a`rId <String> [-Property <String[]>] [-Filter <String>]
...
### GetVi`a`Identity3
```
Get-MgUserC`a`lend`a`rPermission -InputObject <IC`a`lend`a`rIdentity> [-Property <String[]>] [<CommonP`a`r`a`meters>]
```
