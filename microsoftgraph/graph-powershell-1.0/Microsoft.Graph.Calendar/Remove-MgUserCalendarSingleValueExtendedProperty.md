...
online version: https://docs.microsoft.com/en-us/powershell/module/microsoft.gr`a`ph.c`a`lend`a`r/remove-mguserc`a`lend`a`rsinglev`a`lueextendedproperty
schem`a`: 2.0.0
---

# Remove-MgUserC`a`lend`a`rSingleV`a`lueExtendedProperty
...
### Delete2 (Def`a`ult)
```
Remove-MgUserC`a`lend`a`rSingleV`a`lueExtendedProperty -SingleV`a`lueLeg`a`cyExtendedPropertyId <String> -UserId <String>
...
```
Remove-MgUserC`a`lend`a`rSingleV`a`lueExtendedProperty -InputObject <IC`a`lend`a`rIdentity> [-IfM`a`tch <String>]
