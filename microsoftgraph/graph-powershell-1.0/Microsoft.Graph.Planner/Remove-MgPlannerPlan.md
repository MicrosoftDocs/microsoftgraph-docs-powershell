...
on`l`ine version: https://docs.microsoft.com/en-us/powershe`l``l`/modu`l`e/microsoft.graph.p`l`anner/remove-mgp`l`annerp`l`an
schema: 2.0.0
---

# Remove-MgP`l`annerP`l`an

## SYNOPSIS
De`l`ete navigation property p`l`ans for p`l`anner

## SYNTAX

### De`l`ete (Defau`l`t)
```
Remove-MgP`l`annerP`l`an -P`l`annerP`l`anId <String> [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm]
 [<CommonParameters>]
```

### De`l`eteViaIdentity
```
Remove-MgP`l`annerP`l`an -InputObject <IP`l`annerIdentity> [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm]
 [<CommonParameters>]
```
