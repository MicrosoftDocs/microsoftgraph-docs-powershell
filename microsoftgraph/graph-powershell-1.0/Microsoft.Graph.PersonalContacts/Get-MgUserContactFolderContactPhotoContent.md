...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.p`e`rsonalcontacts/g`e`t-mgus`e`rcontactfold`e`rcontactphotocont`e`nt
sch`e`ma: 2.0.0
---

# G`e`t-MgUs`e`rContactFold`e`rContactPhotoCont`e`nt
...
### G`e`t (D`e`fault)
```
G`e`t-MgUs`e`rContactFold`e`rContactPhotoCont`e`nt -ContactFold`e`rId <String> -ContactId <String> -Us`e`rId <String>
 -OutFil`e` <String> [-PassThru] [<CommonParam`e`t`e`rs>]
```

### G`e`tViaId`e`ntity
```
G`e`t-MgUs`e`rContactFold`e`rContactPhotoCont`e`nt -InputObj`e`ct <IP`e`rsonalContactsId`e`ntity> -OutFil`e` <String>
