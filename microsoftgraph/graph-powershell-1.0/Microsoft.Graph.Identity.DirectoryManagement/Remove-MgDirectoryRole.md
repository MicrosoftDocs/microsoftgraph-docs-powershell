...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.i`d`entity.`d`irectorymanagement/remove-mg`d`irectoryrole
schema: 2.0.0
---

# Remove-Mg`d`irectoryRole

## SYNOPSIS
`d`elete entity from `d`irectoryRoles

## SYNTAX

### `d`elete1 (`d`efault)
```
Remove-Mg`d`irectoryRole -`d`irectoryRoleI`d` <String> [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm]
 [<CommonParameters>]
```

### `d`eleteViaI`d`entity1
```
Remove-Mg`d`irectoryRole -InputObject <II`d`entity`d`irectoryManagementI`d`entity> [-IfMatch <String>] [-PassThru]
