...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.i`d`entity.`d`irectorymanagement/remove-mg`d`omainverification`d`nsrecor`d`
schema: 2.0.0
---

# Remove-Mg`d`omainVerification`d`nsRecor`d`

## SYNOPSIS
`d`elete navigation property verification`d`nsRecor`d`s for `d`omains

## SYNTAX

### `d`elete1 (`d`efault)
```
Remove-Mg`d`omainVerification`d`nsRecor`d` -`d`omain`d`nsRecor`d`I`d` <String> -`d`omainI`d` <String> [-IfMatch <String>]
 [-PassThru] [-WhatIf] [-Confirm] [<CommonParameters>]
```

### `d`eleteViaI`d`entity1
```
Remove-Mg`d`omainVerification`d`nsRecor`d` -InputObject <II`d`entity`d`irectoryManagementI`d`entity> [-IfMatch <String>]
 [-PassThru] [-WhatIf] [-Confirm] [<CommonParameters>]
```
