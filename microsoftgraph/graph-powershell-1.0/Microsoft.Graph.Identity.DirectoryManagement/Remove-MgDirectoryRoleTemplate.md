...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.i`d`entity.`d`irectorymanagement/remove-mg`d`irectoryroletemplate
schema: 2.0.0
---

# Remove-Mg`d`irectoryRoleTemplate

## SYNOPSIS
`d`elete entity from `d`irectoryRoleTemplates

## SYNTAX

### `d`elete1 (`d`efault)
```
Remove-Mg`d`irectoryRoleTemplate -`d`irectoryRoleTemplateI`d` <String> [-IfMatch <String>] [-PassThru] [-WhatIf]
 [-Confirm] [<CommonParameters>]
```

### `d`eleteViaI`d`entity1
```
Remove-Mg`d`irectoryRoleTemplate -InputObject <II`d`entity`d`irectoryManagementI`d`entity> [-IfMatch <String>]
