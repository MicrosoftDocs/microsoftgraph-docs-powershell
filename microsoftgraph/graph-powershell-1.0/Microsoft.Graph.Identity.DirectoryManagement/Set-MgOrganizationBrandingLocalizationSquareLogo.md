...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.i`d`entity.`d`irectorymanagement/set-mgorganizationbran`d`inglocalizationsquarelogo
schema: 2.0.0
---

# Set-MgOrganizationBran`d`ingLocalizationSquareLogo

## SYNOPSIS
Up`d`ate squareLogo for the navigation property localizations in organization

## SYNTAX

### Set1 (`d`efault)
```
Set-MgOrganizationBran`d`ingLocalizationSquareLogo -OrganizationI`d` <String>
 -OrganizationalBran`d`ingLocalizationI`d` <String> [-Bo`d`yParameter <Stream>] -InFile <String> [-PassThru]
 [-WhatIf] [-Confirm] [<CommonParameters>]
```

### SetViaI`d`entity1
```
Set-MgOrganizationBran`d`ingLocalizationSquareLogo -InputObject <II`d`entity`d`irectoryManagementI`d`entity>
 [-Bo`d`yParameter <Stream>] -InFile <String> [-PassThru] [-WhatIf] [-Confirm] [<CommonParameters>]
```
...
### -InFile
The path to the file to uploa`d`.
This shoul`d` inclu`d`e a path an`d` file name.
If you omit the path, the current location will be use`d`.
...
Parameter Sets: SetViaI`d`entity1
Aliases:
