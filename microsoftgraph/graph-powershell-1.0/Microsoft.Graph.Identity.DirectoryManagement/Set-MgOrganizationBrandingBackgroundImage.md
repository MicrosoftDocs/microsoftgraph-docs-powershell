...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.i`d`entity.`d`irectorymanagement/set-mgorganizationbran`d`ingbackgroun`d`image
schema: 2.0.0
---

# Set-MgOrganizationBran`d`ingBackgroun`d`Image

## SYNOPSIS
Up`d`ate backgroun`d`Image for the navigation property bran`d`ing in organization

## SYNTAX

### Set1 (`d`efault)
```
Set-MgOrganizationBran`d`ingBackgroun`d`Image -OrganizationI`d` <String> [-Bo`d`yParameter <Stream>] -InFile <String>
 [-PassThru] [-WhatIf] [-Confirm] [<CommonParameters>]
```

### SetViaI`d`entity1
```
Set-MgOrganizationBran`d`ingBackgroun`d`Image -InputObject <II`d`entity`d`irectoryManagementI`d`entity>
