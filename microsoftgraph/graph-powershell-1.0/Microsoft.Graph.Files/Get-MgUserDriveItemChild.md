...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.f`i`les/get-mguserdr`i`ve`i`temch`i`ld
schema: 2.0.0
---

# Get-MgUserDr`i`ve`i`temCh`i`ld

## SYNOPS`i`S
Get ch`i`ldren from users

## SYNTAX

### L`i`st (Default)
```
Get-MgUserDr`i`ve`i`temCh`i`ld -Dr`i`ve`i`d <Str`i`ng> -Dr`i`ve`i`tem`i`d <Str`i`ng> -User`i`d <Str`i`ng> [-ExpandProperty <Str`i`ng[]>]
...
### Get
```
Get-MgUserDr`i`ve`i`temCh`i`ld -Dr`i`ve`i`d <Str`i`ng> -Dr`i`ve`i`tem`i`d <Str`i`ng> -Dr`i`ve`i`tem`i`d1 <Str`i`ng> -User`i`d <Str`i`ng>
 [-ExpandProperty <Str`i`ng[]>] [-Property <Str`i`ng[]>] [<CommonParameters>]
```

### GetV`i`a`i`dent`i`ty
```
Get-MgUserDr`i`ve`i`temCh`i`ld -`i`nputObject <`i`F`i`les`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>] [-Property <Str`i`ng[]>]
 [<CommonParameters>]
```
