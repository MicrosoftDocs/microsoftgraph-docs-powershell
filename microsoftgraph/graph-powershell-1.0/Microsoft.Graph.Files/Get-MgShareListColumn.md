...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.f`i`les/get-mgsharel`i`stcolumn
schema: 2.0.0
---

# Get-MgShareL`i`stColumn
...
### L`i`st (Default)
```
Get-MgShareL`i`stColumn -SharedDr`i`ve`i`tem`i`d <Str`i`ng> [-ExpandProperty <Str`i`ng[]>] [-Property <Str`i`ng[]>]
...
### Get
```
Get-MgShareL`i`stColumn -ColumnDef`i`n`i`t`i`on`i`d <Str`i`ng> -SharedDr`i`ve`i`tem`i`d <Str`i`ng> [-ExpandProperty <Str`i`ng[]>]
 [-Property <Str`i`ng[]>] [<CommonParameters>]
```

### GetV`i`a`i`dent`i`ty
```
Get-MgShareL`i`stColumn -`i`nputObject <`i`F`i`les`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>] [-Property <Str`i`ng[]>]
 [<CommonParameters>]
```
