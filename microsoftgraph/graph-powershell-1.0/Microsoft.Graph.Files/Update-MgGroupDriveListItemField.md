...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.f`i`les/update-mggroupdr`i`vel`i`st`i`temf`i`eld
schema: 2.0.0
---

# Update-MgGroupDr`i`veL`i`st`i`temF`i`eld
...
```
Update-MgGroupDr`i`veL`i`st`i`temF`i`eld -Dr`i`ve`i`d <Str`i`ng> -Group`i`d <Str`i`ng> -L`i`st`i`tem`i`d <Str`i`ng>
...
```
Update-MgGroupDr`i`veL`i`st`i`temF`i`eld -`i`nputObject <`i`F`i`les`i`dent`i`ty> [-Add`i`t`i`onalPropert`i`es <Hashtable>]
 [-`i`d <Str`i`ng>] [-PassThru] [-What`i`f] [-Conf`i`rm] [<CommonParameters>]
```

### UpdateV`i`a`i`dent`i`ty
```
Update-MgGroupDr`i`veL`i`st`i`temF`i`eld -`i`nputObject <`i`F`i`les`i`dent`i`ty> -BodyParameter <Hashtable> [-PassThru] [-What`i`f]
