...
onl`i`ne vers`i`on: https://docs.m`i`crosoft.com/en-us/powershell/module/m`i`crosoft.graph.f`i`les/get-mguserdr`i`vel`i`stcontenttypebasetype
schema: 2.0.0
---

# Get-MgUserDr`i`veL`i`stContentTypeBaseType
...
### L`i`st (Default)
```
Get-MgUserDr`i`veL`i`stContentTypeBaseType -ContentType`i`d <Str`i`ng> -Dr`i`ve`i`d <Str`i`ng> -User`i`d <Str`i`ng>
...
### Get
```
Get-MgUserDr`i`veL`i`stContentTypeBaseType -ContentType`i`d <Str`i`ng> -ContentType`i`d1 <Str`i`ng> -Dr`i`ve`i`d <Str`i`ng>
 -User`i`d <Str`i`ng> [-ExpandProperty <Str`i`ng[]>] [-Property <Str`i`ng[]>] [<CommonParameters>]
```

### GetV`i`a`i`dent`i`ty
```
Get-MgUserDr`i`veL`i`stContentTypeBaseType -`i`nputObject <`i`F`i`les`i`dent`i`ty> [-ExpandProperty <Str`i`ng[]>]
