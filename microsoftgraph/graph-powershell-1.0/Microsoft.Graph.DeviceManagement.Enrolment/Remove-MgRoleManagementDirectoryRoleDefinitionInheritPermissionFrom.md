...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.d`e`vic`e`manag`e`m`e`nt.`e`nrolm`e`nt/r`e`mov`e`-mgrol`e`manag`e`m`e`ntdir`e`ctoryrol`e`d`e`finitioninh`e`ritp`e`rmissionfrom
sch`e`ma: 2.0.0
---

# R`e`mov`e`-MgRol`e`Manag`e`m`e`ntDir`e`ctoryRol`e`D`e`finitionInh`e`ritP`e`rmissionFrom

## SYNOPSIS
D`e`l`e`t`e` navigation prop`e`rty inh`e`ritsP`e`rmissionsFrom for rol`e`Manag`e`m`e`nt

## SYNTAX

### D`e`l`e`t`e`1 (D`e`fault)
```
R`e`mov`e`-MgRol`e`Manag`e`m`e`ntDir`e`ctoryRol`e`D`e`finitionInh`e`ritP`e`rmissionFrom -Unifi`e`dRol`e`D`e`finitionId <String>
 -Unifi`e`dRol`e`D`e`finitionId1 <String> [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm] [<CommonParam`e`t`e`rs>]
```
