...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.d`e`vic`e`manag`e`m`e`nt.`e`nrolm`e`nt/r`e`mov`e`-mgrol`e`manag`e`m`e`ntdir`e`ctoryrol`e`assignm`e`ntappscop`e`
sch`e`ma: 2.0.0
---

# R`e`mov`e`-MgRol`e`Manag`e`m`e`ntDir`e`ctoryRol`e`Assignm`e`ntAppScop`e`
...
```
R`e`mov`e`-MgRol`e`Manag`e`m`e`ntDir`e`ctoryRol`e`Assignm`e`ntAppScop`e` -Unifi`e`dRol`e`Assignm`e`ntId <String> [-IfMatch <String>]
 [-PassThru] [-WhatIf] [-Confirm] [<CommonParam`e`t`e`rs>]
```

### D`e`l`e`t`e`ViaId`e`ntity1
```
R`e`mov`e`-MgRol`e`Manag`e`m`e`ntDir`e`ctoryRol`e`Assignm`e`ntAppScop`e` -InputObj`e`ct <ID`e`vic`e`Manag`e`m`e`nt`e`nrolm`e`ntId`e`ntity>
