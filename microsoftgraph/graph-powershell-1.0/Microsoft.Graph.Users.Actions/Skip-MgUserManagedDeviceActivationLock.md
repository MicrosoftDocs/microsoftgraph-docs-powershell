...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/`s`kip-mgu`s`ermanageddeviceactivationlock
`s`chema: 2.0.0
---

# `s`kip-MgU`s`erManagedDeviceActivationLock

## `s`YNOP`s`I`s`
Invoke action bypa`s``s`ActivationLock

## `s`YNTAX

### Bypa`s``s`1 (Default)
```
`s`kip-MgU`s`erManagedDeviceActivationLock -ManagedDeviceId <`s`tring> -U`s`erId <`s`tring> [-Pa`s``s`Thru] [-WhatIf]
 [-Confirm] [<CommonParameter`s`>]
```

### Bypa`s``s`ViaIdentity1
```
`s`kip-MgU`s`erManagedDeviceActivationLock -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
...
Parameter `s`et`s`: Bypa`s``s`ViaIdentity1
Alia`s`e`s`:
