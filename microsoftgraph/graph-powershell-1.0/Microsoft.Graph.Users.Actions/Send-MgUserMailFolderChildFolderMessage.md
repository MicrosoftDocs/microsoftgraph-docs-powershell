...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/`s`end-mgu`s`ermailfolderchildfolderme`s``s`age
`s`chema: 2.0.0
---

# `s`end-MgU`s`erMailFolderChildFolderMe`s``s`age
...
### `s`end1 (Default)
```
`s`end-MgU`s`erMailFolderChildFolderMe`s``s`age -MailFolderId <`s`tring> -MailFolderId1 <`s`tring> -Me`s``s`ageId <`s`tring>
 -U`s`erId <`s`tring> [-Pa`s``s`Thru] [-WhatIf] [-Confirm] [<CommonParameter`s`>]
```

### `s`endViaIdentity1
```
`s`end-MgU`s`erMailFolderChildFolderMe`s``s`age -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
