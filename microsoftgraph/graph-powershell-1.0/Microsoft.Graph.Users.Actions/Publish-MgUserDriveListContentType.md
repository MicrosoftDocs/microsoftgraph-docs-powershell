...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/publi`s`h-mgu`s`erdriveli`s`tcontenttype
`s`chema: 2.0.0
---

# Publi`s`h-MgU`s`erDriveLi`s`tContentType

## `s`YNOP`s`I`s`
Invoke action publi`s`h

## `s`YNTAX

### Publi`s`h (Default)
```
Publi`s`h-MgU`s`erDriveLi`s`tContentType -ContentTypeId <`s`tring> -DriveId <`s`tring> -U`s`erId <`s`tring> [-Pa`s``s`Thru]
 [-WhatIf] [-Confirm] [<CommonParameter`s`>]
```

### Publi`s`hViaIdentity
```
Publi`s`h-MgU`s`erDriveLi`s`tContentType -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
...
Parameter `s`et`s`: Publi`s`hViaIdentity
Alia`s`e`s`:
