...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/invoke-mgretireu`s`ermanageddevice
`s`chema: 2.0.0
---

# Invoke-MgRetireU`s`erManagedDevice
...
### Retire1 (Default)
```
Invoke-MgRetireU`s`erManagedDevice -ManagedDeviceId <`s`tring> -U`s`erId <`s`tring> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```

### RetireViaIdentity1
```
Invoke-MgRetireU`s`erManagedDevice -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
...
Parameter `s`et`s`: RetireViaIdentity1
Alia`s`e`s`:
