...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/re`s`tore-mgu`s`erauthenticationwindowhelloforbu`s`ine`s``s`methoddevice
`s`chema: 2.0.0
---

# Re`s`tore-MgU`s`erAuthenticationWindowHelloForBu`s`ine`s``s`MethodDevice
...
### Re`s`tore1 (Default)
```
Re`s`tore-MgU`s`erAuthenticationWindowHelloForBu`s`ine`s``s`MethodDevice -U`s`erId <`s`tring>
 -Window`s`HelloForBu`s`ine`s``s`AuthenticationMethodId <`s`tring> [-WhatIf] [-Confirm] [<CommonParameter`s`>]
```

### Re`s`toreViaIdentity1
```
Re`s`tore-MgU`s`erAuthenticationWindowHelloForBu`s`ine`s``s`MethodDevice -InputObject <IU`s`er`s`Action`s`Identity> [-WhatIf]
...
Parameter `s`et`s`: Re`s`toreViaIdentity1
Alia`s`e`s`:
