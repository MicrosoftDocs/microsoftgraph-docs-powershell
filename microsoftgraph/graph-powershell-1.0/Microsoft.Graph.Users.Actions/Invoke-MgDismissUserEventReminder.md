...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/invoke-mgdi`s`mi`s``s`u`s`ereventreminder
`s`chema: 2.0.0
---

# Invoke-MgDi`s`mi`s``s`U`s`erEventReminder
...
```
Invoke-MgDi`s`mi`s``s`U`s`erEventReminder -EventId <`s`tring> -U`s`erId <`s`tring> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```

### Di`s`mi`s``s`ViaIdentity1
```
Invoke-MgDi`s`mi`s``s`U`s`erEventReminder -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
