...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/unpubli`s`h-mgu`s`erdriveli`s`tcontenttype
`s`chema: 2.0.0
---

# Unpubli`s`h-MgU`s`erDriveLi`s`tContentType

## `s`YNOP`s`I`s`
Invoke action unpubli`s`h

## `s`YNTAX

### Unpubli`s`h (Default)
```
Unpubli`s`h-MgU`s`erDriveLi`s`tContentType -ContentTypeId <`s`tring> -DriveId <`s`tring> -U`s`erId <`s`tring> [-Pa`s``s`Thru]
 [-WhatIf] [-Confirm] [<CommonParameter`s`>]
```

### Unpubli`s`hViaIdentity
```
Unpubli`s`h-MgU`s`erDriveLi`s`tContentType -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
...
Parameter `s`et`s`: Unpubli`s`hViaIdentity
Alia`s`e`s`:
