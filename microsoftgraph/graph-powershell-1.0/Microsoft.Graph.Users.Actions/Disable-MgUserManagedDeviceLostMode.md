...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/di`s`able-mgu`s`ermanageddevicelo`s`tmode
`s`chema: 2.0.0
---

# Di`s`able-MgU`s`erManagedDeviceLo`s`tMode

## `s`YNOP`s`I`s`
Invoke action di`s`ableLo`s`tMode

## `s`YNTAX

### Di`s`able1 (Default)
```
Di`s`able-MgU`s`erManagedDeviceLo`s`tMode -ManagedDeviceId <`s`tring> -U`s`erId <`s`tring> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```

### Di`s`ableViaIdentity1
```
Di`s`able-MgU`s`erManagedDeviceLo`s`tMode -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
...
Parameter `s`et`s`: Di`s`ableViaIdentity1
Alia`s`e`s`:
