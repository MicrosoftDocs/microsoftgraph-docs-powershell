...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/invoke-mgreauthorizeu`s`erdriveitem`s`ub`s`cription
`s`chema: 2.0.0
---

# Invoke-MgReauthorizeU`s`erDriveItem`s`ub`s`cription
...
### Reauthorize (Default)
```
Invoke-MgReauthorizeU`s`erDriveItem`s`ub`s`cription -DriveId <`s`tring> -DriveItemId <`s`tring> -`s`ub`s`criptionId <`s`tring>
 -U`s`erId <`s`tring> [-Pa`s``s`Thru] [-WhatIf] [-Confirm] [<CommonParameter`s`>]
```

### ReauthorizeViaIdentity
```
Invoke-MgReauthorizeU`s`erDriveItem`s`ub`s`cription -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf]
