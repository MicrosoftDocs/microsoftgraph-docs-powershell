...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/`s`ync-mgu`s`ermanageddevice
`s`chema: 2.0.0
---

# `s`ync-MgU`s`erManagedDevice

## `s`YNOP`s`I`s`
Invoke action `s`yncDevice

## `s`YNTAX

### `s`ync1 (Default)
```
`s`ync-MgU`s`erManagedDevice -ManagedDeviceId <`s`tring> -U`s`erId <`s`tring> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```

### `s`yncViaIdentity1
```
`s`ync-MgU`s`erManagedDevice -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
...
Parameter `s`et`s`: `s`yncViaIdentity1
Alia`s`e`s`:
