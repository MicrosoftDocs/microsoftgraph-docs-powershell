...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/re`s`et-mgu`s`ermanageddevicepa`s``s`code
`s`chema: 2.0.0
---

# Re`s`et-MgU`s`erManagedDevicePa`s``s`code

## `s`YNOP`s`I`s`
Invoke action re`s`etPa`s``s`code

## `s`YNTAX

### Re`s`et1 (Default)
```
Re`s`et-MgU`s`erManagedDevicePa`s``s`code -ManagedDeviceId <`s`tring> -U`s`erId <`s`tring> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```

### Re`s`etViaIdentity1
```
Re`s`et-MgU`s`erManagedDevicePa`s``s`code -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
...
Parameter `s`et`s`: Re`s`etViaIdentity1
Alia`s`e`s`:
