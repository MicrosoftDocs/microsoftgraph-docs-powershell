...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/invoke-mgcheckoutu`s`erdriveitem
`s`chema: 2.0.0
---

# Invoke-MgCheckoutU`s`erDriveItem
...
### Checkout (Default)
```
Invoke-MgCheckoutU`s`erDriveItem -DriveId <`s`tring> -DriveItemId <`s`tring> -U`s`erId <`s`tring> [-Pa`s``s`Thru] [-WhatIf]
 [-Confirm] [<CommonParameter`s`>]
```

### CheckoutViaIdentity
```
Invoke-MgCheckoutU`s`erDriveItem -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
...
Parameter `s`et`s`: CheckoutViaIdentity
Alia`s`e`s`:
