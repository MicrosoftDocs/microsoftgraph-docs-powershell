...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/invoke-mgreauthorizeu`s`erdriveli`s`t`s`ub`s`cription
`s`chema: 2.0.0
---

# Invoke-MgReauthorizeU`s`erDriveLi`s`t`s`ub`s`cription
...
### Reauthorize (Default)
```
Invoke-MgReauthorizeU`s`erDriveLi`s`t`s`ub`s`cription -DriveId <`s`tring> -`s`ub`s`criptionId <`s`tring> -U`s`erId <`s`tring>
 [-Pa`s``s`Thru] [-WhatIf] [-Confirm] [<CommonParameter`s`>]
```

### ReauthorizeViaIdentity
```
Invoke-MgReauthorizeU`s`erDriveLi`s`t`s`ub`s`cription -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf]
...
Parameter `s`et`s`: ReauthorizeViaIdentity
Alia`s`e`s`:
