...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/invoke-mgunfollowu`s`erdriveitem
`s`chema: 2.0.0
---

# Invoke-MgUnfollowU`s`erDriveItem
...
### Unfollow (Default)
```
Invoke-MgUnfollowU`s`erDriveItem -DriveId <`s`tring> -DriveItemId <`s`tring> -U`s`erId <`s`tring> [-Pa`s``s`Thru] [-WhatIf]
 [-Confirm] [<CommonParameter`s`>]
```

### UnfollowViaIdentity
```
Invoke-MgUnfollowU`s`erDriveItem -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
