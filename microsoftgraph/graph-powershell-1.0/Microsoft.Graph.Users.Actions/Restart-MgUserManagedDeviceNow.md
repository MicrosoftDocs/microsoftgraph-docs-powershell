...
online ver`s`ion: http`s`://doc`s`.micro`s`oft.com/en-u`s`/power`s`hell/module/micro`s`oft.graph.u`s`er`s`.action`s`/re`s`tart-mgu`s`ermanageddevicenow
`s`chema: 2.0.0
---

# Re`s`tart-MgU`s`erManagedDeviceNow
...
### Reboot1 (Default)
```
Re`s`tart-MgU`s`erManagedDeviceNow -ManagedDeviceId <`s`tring> -U`s`erId <`s`tring> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```

### RebootViaIdentity1
```
Re`s`tart-MgU`s`erManagedDeviceNow -InputObject <IU`s`er`s`Action`s`Identity> [-Pa`s``s`Thru] [-WhatIf] [-Confirm]
 [<CommonParameter`s`>]
```
...
Parameter `s`et`s`: RebootViaIdentity1
Alia`s`e`s`:
