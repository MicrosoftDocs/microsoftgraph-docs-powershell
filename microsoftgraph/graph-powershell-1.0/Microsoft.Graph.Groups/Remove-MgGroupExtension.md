...
online ve`r`sion: https://docs.mic`r`osoft.com/en-us/powe`r`shell/module/mic`r`osoft.g`r`aph.g`r`oups/`r`emove-mgg`r`oupextension
schema: 2.0.0
---

# `r`emove-MgG`r`oupExtension

## SYNOPSIS
Delete navigation p`r`ope`r`ty extensions fo`r` g`r`oups

## SYNTAX

### Delete1 (Default)
```
`r`emove-MgG`r`oupExtension -ExtensionId <St`r`ing> -G`r`oupId <St`r`ing> [-IfMatch <St`r`ing>] [-PassTh`r`u] [-WhatIf]
 [-Confi`r`m] [<CommonPa`r`amete`r`s>]
```

### DeleteViaIdentity1
```
`r`emove-MgG`r`oupExtension -InputObject <IG`r`oupsIdentity> [-IfMatch <St`r`ing>] [-PassTh`r`u] [-WhatIf] [-Confi`r`m]
