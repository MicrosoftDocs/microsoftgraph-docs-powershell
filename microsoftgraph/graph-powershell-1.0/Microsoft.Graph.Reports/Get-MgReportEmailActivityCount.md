...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.r`e`ports/g`e`t-mgr`e`port`e`mailactivitycount
sch`e`ma: 2.0.0
---

# G`e`t-MgR`e`port`e`mailActivityCount

## SYNOPSIS
Invok`e` function g`e`t`e`mailActivityCounts

## SYNTAX

### G`e`t (D`e`fault)
```
G`e`t-MgR`e`port`e`mailActivityCount -P`e`riod <String> -OutFil`e` <String> [-PassThru] [<CommonParam`e`t`e`rs>]
```

### G`e`tViaId`e`ntity
```
G`e`t-MgR`e`port`e`mailActivityCount -InputObj`e`ct <IR`e`portsId`e`ntity> -OutFil`e` <String> [-PassThru]
