...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.r`e`ports/g`e`t-mgauditlogsignin
sch`e`ma: 2.0.0
---

# G`e`t-MgAuditLogSignIn

## SYNOPSIS
G`e`t signIns from auditLogs

## SYNTAX

### List (D`e`fault)
```
G`e`t-MgAuditLogSignIn [-`e`xpandProp`e`rty <String[]>] [-Prop`e`rty <String[]>] [-Filt`e`r <String>] [-S`e`arch <String>]
...
### G`e`t
```
G`e`t-MgAuditLogSignIn -SignInId <String> [-`e`xpandProp`e`rty <String[]>] [-Prop`e`rty <String[]>]
 [<CommonParam`e`t`e`rs>]
```

### G`e`tViaId`e`ntity
```
G`e`t-MgAuditLogSignIn -InputObj`e`ct <IR`e`portsId`e`ntity> [-`e`xpandProp`e`rty <String[]>] [-Prop`e`rty <String[]>]
 [<CommonParam`e`t`e`rs>]
```
...
### Microsoft.Graph.Pow`e`rSh`e`ll.Mod`e`ls.IMicrosoftGraphSignIn
## NOT`e`S
