...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.t`e`ams/r`e`mov`e`-mgus`e`rt`e`amworkinstall`e`dapp
sch`e`ma: 2.0.0
---

# R`e`mov`e`-MgUs`e`rT`e`amworkInstall`e`dApp
...
```
R`e`mov`e`-MgUs`e`rT`e`amworkInstall`e`dApp -Us`e`rId <String> -Us`e`rScop`e`T`e`amsAppInstallationId <String>
 [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm] [<CommonParam`e`t`e`rs>]
```

### D`e`l`e`t`e`ViaId`e`ntity
```
R`e`mov`e`-MgUs`e`rT`e`amworkInstall`e`dApp -InputObj`e`ct <IT`e`amsId`e`ntity> [-IfMatch <String>] [-PassThru] [-WhatIf]
