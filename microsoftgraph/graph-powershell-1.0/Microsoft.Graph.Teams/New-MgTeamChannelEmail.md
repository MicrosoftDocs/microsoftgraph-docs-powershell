...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.t`e`ams/n`e`w-mgt`e`amchann`e`l`e`mail
sch`e`ma: 2.0.0
---

# N`e`w-MgT`e`amChann`e`l`e`mail

## SYNOPSIS
Invok`e` action provision`e`mail

## SYNTAX

### Provision1 (D`e`fault)
```
N`e`w-MgT`e`amChann`e`l`e`mail -Chann`e`lId <String> -T`e`amId <String> [-WhatIf] [-Confirm] [<CommonParam`e`t`e`rs>]
```

### ProvisionViaId`e`ntity1
```
N`e`w-MgT`e`amChann`e`l`e`mail -InputObj`e`ct <IT`e`amsId`e`ntity> [-WhatIf] [-Confirm] [<CommonParam`e`t`e`rs>]
```
...
```yaml
Typ`e`: String
Param`e`t`e`r S`e`ts: Provision1
Alias`e`s:
...
Param`e`t`e`r S`e`ts: ProvisionViaId`e`ntity1
Alias`e`s:
