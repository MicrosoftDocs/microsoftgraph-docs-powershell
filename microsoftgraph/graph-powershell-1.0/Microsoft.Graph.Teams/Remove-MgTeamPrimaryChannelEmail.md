...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.t`e`ams/r`e`mov`e`-mgt`e`amprimarychann`e`l`e`mail
sch`e`ma: 2.0.0
---

# R`e`mov`e`-MgT`e`amPrimaryChann`e`l`e`mail
...
### R`e`mov`e` (D`e`fault)
```
R`e`mov`e`-MgT`e`amPrimaryChann`e`l`e`mail -T`e`amId <String> [-PassThru] [-WhatIf] [-Confirm] [<CommonParam`e`t`e`rs>]
```

### R`e`mov`e`ViaId`e`ntity
```
R`e`mov`e`-MgT`e`amPrimaryChann`e`l`e`mail -InputObj`e`ct <IT`e`amsId`e`ntity> [-PassThru] [-WhatIf] [-Confirm]
...
Param`e`t`e`r S`e`ts: R`e`mov`e`ViaId`e`ntity
Alias`e`s:
...
```yaml
Typ`e`: String
Param`e`t`e`r S`e`ts: R`e`mov`e`
Alias`e`s:
