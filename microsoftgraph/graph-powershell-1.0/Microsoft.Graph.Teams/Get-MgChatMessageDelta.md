...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.t`e`ams/g`e`t-mgchatm`e`ssag`e`d`e`lta
sch`e`ma: 2.0.0
---

# G`e`t-MgChatM`e`ssag`e`D`e`lta
...
### D`e`lta1 (D`e`fault)
```
G`e`t-MgChatM`e`ssag`e`D`e`lta -ChatId <String> [<CommonParam`e`t`e`rs>]
```

### D`e`ltaViaId`e`ntity1
```
G`e`t-MgChatM`e`ssag`e`D`e`lta -InputObj`e`ct <IT`e`amsId`e`ntity> [<CommonParam`e`t`e`rs>]
```
