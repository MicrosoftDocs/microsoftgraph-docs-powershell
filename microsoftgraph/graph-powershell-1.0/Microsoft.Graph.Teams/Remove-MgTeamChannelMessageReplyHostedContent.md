...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.t`e`ams/r`e`mov`e`-mgt`e`amchann`e`lm`e`ssag`e`r`e`plyhost`e`dcont`e`nt
sch`e`ma: 2.0.0
---

# R`e`mov`e`-MgT`e`amChann`e`lM`e`ssag`e`R`e`plyHost`e`dCont`e`nt
...
```
R`e`mov`e`-MgT`e`amChann`e`lM`e`ssag`e`R`e`plyHost`e`dCont`e`nt -Chann`e`lId <String> -ChatM`e`ssag`e`Host`e`dCont`e`ntId <String>
...
```
R`e`mov`e`-MgT`e`amChann`e`lM`e`ssag`e`R`e`plyHost`e`dCont`e`nt -InputObj`e`ct <IT`e`amsId`e`ntity> [-IfMatch <String>] [-PassThru]
