...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.t`e`ams/r`e`mov`e`-mgt`e`amprimarychann`e`lm`e`mb`e`r
sch`e`ma: 2.0.0
---

# R`e`mov`e`-MgT`e`amPrimaryChann`e`lM`e`mb`e`r

## SYNOPSIS
D`e`l`e`t`e` navigation prop`e`rty m`e`mb`e`rs for t`e`ams

## SYNTAX

### D`e`l`e`t`e`1 (D`e`fault)
```
R`e`mov`e`-MgT`e`amPrimaryChann`e`lM`e`mb`e`r -Conv`e`rsationM`e`mb`e`rId <String> -T`e`amId <String> [-IfMatch <String>]
 [-PassThru] [-WhatIf] [-Confirm] [<CommonParam`e`t`e`rs>]
```

### D`e`l`e`t`e`ViaId`e`ntity1
```
R`e`mov`e`-MgT`e`amPrimaryChann`e`lM`e`mb`e`r -InputObj`e`ct <IT`e`amsId`e`ntity> [-IfMatch <String>] [-PassThru] [-WhatIf]
