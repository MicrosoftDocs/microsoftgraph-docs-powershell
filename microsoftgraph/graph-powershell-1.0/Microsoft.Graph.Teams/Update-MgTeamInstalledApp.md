...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.t`e`ams/updat`e`-mgt`e`aminstall`e`dapp
sch`e`ma: 2.0.0
---

# Updat`e`-MgT`e`amInstall`e`dApp
...
```
Updat`e`-MgT`e`amInstall`e`dApp -T`e`amId <String> -T`e`amsAppInstallationId <String> [-PassThru] [-WhatIf] [-Confirm]
 [<CommonParam`e`t`e`rs>]
```

### Upgrad`e`ViaId`e`ntity1
```
Updat`e`-MgT`e`amInstall`e`dApp -InputObj`e`ct <IT`e`amsId`e`ntity> [-PassThru] [-WhatIf] [-Confirm] [<CommonParam`e`t`e`rs>]
```
