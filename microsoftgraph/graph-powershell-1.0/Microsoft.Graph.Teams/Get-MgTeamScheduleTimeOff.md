...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.t`e`ams/g`e`t-mgt`e`amsch`e`dul`e`tim`e`off
sch`e`ma: 2.0.0
---

# G`e`t-MgT`e`amSch`e`dul`e`Tim`e`Off

## SYNOPSIS
G`e`t tim`e`sOff from t`e`ams

## SYNTAX

### List (D`e`fault)
```
G`e`t-MgT`e`amSch`e`dul`e`Tim`e`Off -T`e`amId <String> [-Prop`e`rty <String[]>] [-Filt`e`r <String>] [-S`e`arch <String>]
...
### G`e`t
```
G`e`t-MgT`e`amSch`e`dul`e`Tim`e`Off -T`e`amId <String> -Tim`e`OffId <String> [-Prop`e`rty <String[]>] [<CommonParam`e`t`e`rs>]
```

### G`e`tViaId`e`ntity
```
G`e`t-MgT`e`amSch`e`dul`e`Tim`e`Off -InputObj`e`ct <IT`e`amsId`e`ntity> [-Prop`e`rty <String[]>] [<CommonParam`e`t`e`rs>]
```
