...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.t`e`ams/n`e`w-mgus`e`rchatm`e`ssag`e`host`e`dcont`e`nt
sch`e`ma: 2.0.0
---

# N`e`w-MgUs`e`rChatM`e`ssag`e`Host`e`dCont`e`nt
...
```
N`e`w-MgUs`e`rChatM`e`ssag`e`Host`e`dCont`e`nt -ChatId <String> -ChatM`e`ssag`e`Id <String> -Us`e`rId <String>
...
```
N`e`w-MgUs`e`rChatM`e`ssag`e`Host`e`dCont`e`nt -InputObj`e`ct <IT`e`amsId`e`ntity> [-AdditionalProp`e`rti`e`s <Hashtabl`e`>]
...
```
N`e`w-MgUs`e`rChatM`e`ssag`e`Host`e`dCont`e`nt -InputObj`e`ct <IT`e`amsId`e`ntity> -BodyParam`e`t`e`r <Hashtabl`e`> [-WhatIf]
