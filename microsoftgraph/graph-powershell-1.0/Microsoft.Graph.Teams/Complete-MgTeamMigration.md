...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.t`e`ams/compl`e`t`e`-mgt`e`ammigration
sch`e`ma: 2.0.0
---

# Compl`e`t`e`-MgT`e`amMigration
...
```
Compl`e`t`e`-MgT`e`amMigration -T`e`amId <String> [-PassThru] [-WhatIf] [-Confirm] [<CommonParam`e`t`e`rs>]
```

### Compl`e`t`e`ViaId`e`ntity1
```
Compl`e`t`e`-MgT`e`amMigration -InputObj`e`ct <IT`e`amsId`e`ntity> [-PassThru] [-WhatIf] [-Confirm] [<CommonParam`e`t`e`rs>]
```
