...
onlin`e` v`e`rsion: https://docs.microsoft.com/`e`n-us/pow`e`rsh`e`ll/modul`e`/microsoft.graph.t`e`ams/r`e`mov`e`-mgchat
sch`e`ma: 2.0.0
---
...
## SYNOPSIS
D`e`l`e`t`e` `e`ntity from chats

## SYNTAX

### D`e`l`e`t`e`1 (D`e`fault)
```
R`e`mov`e`-MgChat -ChatId <String> [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm] [<CommonParam`e`t`e`rs>]
```

### D`e`l`e`t`e`ViaId`e`ntity1
```
R`e`mov`e`-MgChat -InputObj`e`ct <IT`e`amsId`e`ntity> [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm]
