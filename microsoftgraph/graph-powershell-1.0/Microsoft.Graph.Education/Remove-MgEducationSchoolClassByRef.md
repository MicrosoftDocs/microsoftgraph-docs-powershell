...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.e`d`ucation/remove-mge`d`ucationschoolclassbyref
schema: 2.0.0
---

# Remove-MgE`d`ucationSchoolClassByRef

## SYNOPSIS
`d`elete ref of navigation property classes for e`d`ucation

## SYNTAX

### `d`elete1 (`d`efault)
```
Remove-MgE`d`ucationSchoolClassByRef -E`d`ucationClassI`d` <String> -E`d`ucationSchoolI`d` <String> [-I`d` <String>]
 [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm] [<CommonParameters>]
```

### `d`eleteViaI`d`entity1
```
Remove-MgE`d`ucationSchoolClassByRef -InputObject <IE`d`ucationI`d`entity> [-I`d` <String>] [-IfMatch <String>]
