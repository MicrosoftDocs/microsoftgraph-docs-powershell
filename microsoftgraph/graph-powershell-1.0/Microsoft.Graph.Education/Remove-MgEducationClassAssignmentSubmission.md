...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.e`d`ucation/remove-mge`d`ucationclassassignmentsubmission
schema: 2.0.0
---

# Remove-MgE`d`ucationClassAssignmentSubmission
...
### `d`elete (`d`efault)
```
Remove-MgE`d`ucationClassAssignmentSubmission -E`d`ucationAssignmentI`d` <String> -E`d`ucationClassI`d` <String>
 -E`d`ucationSubmissionI`d` <String> [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm] [<CommonParameters>]
```

### `d`eleteViaI`d`entity
```
Remove-MgE`d`ucationClassAssignmentSubmission -InputObject <IE`d`ucationI`d`entity> [-IfMatch <String>] [-PassThru]
