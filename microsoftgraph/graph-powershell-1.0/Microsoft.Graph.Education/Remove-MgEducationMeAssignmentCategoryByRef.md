...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.e`d`ucation/remove-mge`d`ucationmeassignmentcategorybyref
schema: 2.0.0
---

# Remove-MgE`d`ucationMeAssignmentCategoryByRef

## SYNOPSIS
`d`elete ref of navigation property categories for e`d`ucation

## SYNTAX

### `d`elete (`d`efault)
```
Remove-MgE`d`ucationMeAssignmentCategoryByRef -E`d`ucationAssignmentI`d` <String> -E`d`ucationCategoryI`d` <String>
 [-I`d` <String>] [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm] [<CommonParameters>]
```

### `d`eleteViaI`d`entity
```
Remove-MgE`d`ucationMeAssignmentCategoryByRef -InputObject <IE`d`ucationI`d`entity> [-I`d` <String>]
