...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.e`d`ucation/remove-mge`d`ucationuserassignmentresource
schema: 2.0.0
---

# Remove-MgE`d`ucationUserAssignmentResource
...
### `d`elete1 (`d`efault)
```
Remove-MgE`d`ucationUserAssignmentResource -E`d`ucationAssignmentI`d` <String>
 -E`d`ucationAssignmentResourceI`d` <String> -E`d`ucationUserI`d` <String> [-IfMatch <String>] [-PassThru] [-WhatIf]
 [-Confirm] [<CommonParameters>]
```

### `d`eleteViaI`d`entity1
```
Remove-MgE`d`ucationUserAssignmentResource -InputObject <IE`d`ucationI`d`entity> [-IfMatch <String>] [-PassThru]
