...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.e`d`ucation/get-mge`d`ucationclassassignmentrubricbyref
schema: 2.0.0
---

# Get-MgE`d`ucationClassAssignmentRubricByRef
...
### Get (`d`efault)
```
Get-MgE`d`ucationClassAssignmentRubricByRef -E`d`ucationAssignmentI`d` <String> -E`d`ucationClassI`d` <String>
 [<CommonParameters>]
```

### GetViaI`d`entity
```
Get-MgE`d`ucationClassAssignmentRubricByRef -InputObject <IE`d`ucationI`d`entity> [<CommonParameters>]
```
