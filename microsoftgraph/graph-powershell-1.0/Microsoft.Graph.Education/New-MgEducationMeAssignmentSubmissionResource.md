...
online version: https://```d```ocs.microsoft.com/en-us/powershell/mo```d```ule/microsoft.graph.e```d```ucation/new-mge```d```ucationmeassignmentsubmissionresource
schema: 2.0.0
---

# New-MgE```d```ucationMeAssignmentSubmissionResource
...
```
New-MgE```d```ucationMeAssignmentSubmissionResource -E```d```ucationAssignmentI```d``` <String> -E```d```ucationSubmissionI```d``` <String>
...
```
New-MgE```d```ucationMeAssignmentSubmissionResource -InputObject <IE```d```ucationI```d```entity>
 [-A```d``````d```itionalProperties <Hashtable>] [-AssignmentResourceUrl <String>] [-I```d``` <String>]
 [-Resource <IMicrosoftGraphE```d```ucationResource>] [-WhatIf] [-Confirm] [<CommonParameters>]
```
