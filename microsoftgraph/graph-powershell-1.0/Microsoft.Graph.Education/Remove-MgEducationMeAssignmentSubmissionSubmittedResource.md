...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.e`d`ucation/remove-mge`d`ucationmeassignmentsubmissionsubmitte`d`resource
schema: 2.0.0
---

# Remove-MgE`d`ucationMeAssignmentSubmissionSubmitte`d`Resource

## SYNOPSIS
`d`elete navigation property submitte`d`Resources for e`d`ucation

## SYNTAX

### `d`elete1 (`d`efault)
```
Remove-MgE`d`ucationMeAssignmentSubmissionSubmitte`d`Resource -E`d`ucationAssignmentI`d` <String>
...
```
Remove-MgE`d`ucationMeAssignmentSubmissionSubmitte`d`Resource -InputObject <IE`d`ucationI`d`entity> [-IfMatch <String>]
