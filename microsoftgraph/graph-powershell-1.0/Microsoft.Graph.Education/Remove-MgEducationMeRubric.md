...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.e`d`ucation/remove-mge`d`ucationmerubric
schema: 2.0.0
---

# Remove-MgE`d`ucationMeRubric

## SYNOPSIS
`d`elete navigation property rubrics for e`d`ucation

## SYNTAX

### `d`elete1 (`d`efault)
```
Remove-MgE`d`ucationMeRubric -E`d`ucationRubricI`d` <String> [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm]
 [<CommonParameters>]
```

### `d`eleteViaI`d`entity1
```
Remove-MgE`d`ucationMeRubric -InputObject <IE`d`ucationI`d`entity> [-IfMatch <String>] [-PassThru] [-WhatIf]
