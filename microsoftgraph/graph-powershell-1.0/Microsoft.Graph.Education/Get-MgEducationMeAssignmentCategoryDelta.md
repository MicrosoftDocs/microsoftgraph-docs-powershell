...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.e`d`ucation/get-mge`d`ucationmeassignmentcategory`d`elta
schema: 2.0.0
---

# Get-MgE`d`ucationMeAssignmentCategory`d`elta
...
### `d`elta (`d`efault)
```
Get-MgE`d`ucationMeAssignmentCategory`d`elta -E`d`ucationAssignmentI`d` <String> [<CommonParameters>]
```

### `d`eltaViaI`d`entity
```
Get-MgE`d`ucationMeAssignmentCategory`d`elta -InputObject <IE`d`ucationI`d`entity> [<CommonParameters>]
```
...
Parameter Sets: `d`eltaViaI`d`entity
Aliases:
