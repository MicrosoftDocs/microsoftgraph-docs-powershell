...
online version: https://```d```ocs.microsoft.com/en-us/powershell/mo```d```ule/microsoft.graph.e```d```ucation/new-mge```d```ucationmeassignmentsubmissionsubmitte```d```resource
schema: 2.0.0
---

# New-MgE```d```ucationMeAssignmentSubmissionSubmitte```d```Resource
...
```
New-MgE```d```ucationMeAssignmentSubmissionSubmitte```d```Resource -E```d```ucationAssignmentI```d``` <String>
...
```
New-MgE```d```ucationMeAssignmentSubmissionSubmitte```d```Resource -InputObject <IE```d```ucationI```d```entity>
