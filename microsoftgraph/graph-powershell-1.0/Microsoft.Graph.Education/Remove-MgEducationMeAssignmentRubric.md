...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.e`d`ucation/remove-mge`d`ucationmeassignmentrubric
schema: 2.0.0
---

# Remove-MgE`d`ucationMeAssignmentRubric

## SYNOPSIS
`d`elete navigation property rubric for e`d`ucation

## SYNTAX

### `d`elete1 (`d`efault)
```
Remove-MgE`d`ucationMeAssignmentRubric -E`d`ucationAssignmentI`d` <String> [-IfMatch <String>] [-PassThru] [-WhatIf]
 [-Confirm] [<CommonParameters>]
```

### `d`eleteViaI`d`entity1
```
Remove-MgE`d`ucationMeAssignmentRubric -InputObject <IE`d`ucationI`d`entity> [-IfMatch <String>] [-PassThru]
