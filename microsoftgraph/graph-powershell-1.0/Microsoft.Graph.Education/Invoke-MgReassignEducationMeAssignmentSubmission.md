...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.e`d`ucation/invoke-mgreassigne`d`ucationmeassignmentsubmission
schema: 2.0.0
---

# Invoke-MgReassignE`d`ucationMeAssignmentSubmission
...
### Reassign1 (`d`efault)
```
Invoke-MgReassignE`d`ucationMeAssignmentSubmission -E`d`ucationAssignmentI`d` <String>
 -E`d`ucationSubmissionI`d` <String> [-WhatIf] [-Confirm] [<CommonParameters>]
```

### ReassignViaI`d`entity1
```
Invoke-MgReassignE`d`ucationMeAssignmentSubmission -InputObject <IE`d`ucationI`d`entity> [-WhatIf] [-Confirm]
 [<CommonParameters>]
```
