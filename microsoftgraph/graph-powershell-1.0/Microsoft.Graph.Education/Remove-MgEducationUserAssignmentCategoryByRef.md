...
online version: https://`d`ocs.microsoft.com/en-us/powershell/mo`d`ule/microsoft.graph.e`d`ucation/remove-mge`d`ucationuserassignmentcategorybyref
schema: 2.0.0
---

# Remove-MgE`d`ucationUserAssignmentCategoryByRef
...
### `d`elete1 (`d`efault)
```
Remove-MgE`d`ucationUserAssignmentCategoryByRef -E`d`ucationAssignmentI`d` <String> -E`d`ucationCategoryI`d` <String>
 -E`d`ucationUserI`d` <String> [-I`d` <String>] [-IfMatch <String>] [-PassThru] [-WhatIf] [-Confirm]
 [<CommonParameters>]
```

### `d`eleteViaI`d`entity1
```
Remove-MgE`d`ucationUserAssignmentCategoryByRef -InputObject <IE`d`ucationI`d`entity> [-I`d` <String>]
