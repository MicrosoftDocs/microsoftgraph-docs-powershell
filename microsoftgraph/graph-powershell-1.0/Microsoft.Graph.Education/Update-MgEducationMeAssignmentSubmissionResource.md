...
online version: https://```d```ocs.microsoft.com/en-us/powershell/mo```d```ule/microsoft.graph.e```d```ucation/up```d```ate-mge```d```ucationmeassignmentsubmissionresource
schema: 2.0.0
---

# Up```d```ate-MgE```d```ucationMeAssignmentSubmissionResource
...
```
Up```d```ate-MgE```d```ucationMeAssignmentSubmissionResource -E```d```ucationAssignmentI```d``` <String>
...
```
Up```d```ate-MgE```d```ucationMeAssignmentSubmissionResource -InputObject <IE```d```ucationI```d```entity>
